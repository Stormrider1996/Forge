<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <select id="country" name="country" class="block w-full mt-1" required>
                <option value="" selected disabled>{{ __('Select your country') }}</option>
                @foreach ($countries as $key => $country)
                <option value={{ $key }} {{ old('country', $user->country) === $key ? 'selected' : '' }}>{{
                    $country }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="language" name="language" class="block w-full mt-1" required>
                <option value="" selected disabled>{{ __('Select your language') }}</option>
